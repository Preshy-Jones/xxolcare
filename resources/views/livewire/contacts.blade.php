<section class="pt-14 relative pb-56">
    ...
    <div class="flex items-center md:items-start flex-col md:flex-row">
        <div id="change" class="md:flex-2 pl-6 py-8 font-semibold text-opacity-50 text-black ">
            ...
            <a href="{{ url('/bookings/schedule/'.$service) }}">
            ...
            <h2 class="cursor-pointer mb-8 text-sm text-secondary"><span
                    class=" bg-secondary text-sm py-0.5 px-2 rounded-secondary  bg-opacity-50">3</span>
                Contact details
            </h2>
            <h2 class="cursor-pointer mb-8 text-sm"><span
        ...
        <div id="contact_block" class="md:flex-4 md:mr-10 shadow-bigCard px-6 rounded-lgx pb-16 max-h-96">
            ...
            <form class="flex flex-col text-sm text-fifth" 
            wire:submit.prevent="contactSubmit"
            {{-- method='POST' action="/contactdetails/{{$service}}/{{session('booking')->id}}" --}}
            >
                {{-- @csrf
                @method('PUT')
                ...
                <div class='flex flex-col mb-4'>
                    ...
                    <input name='phone' class="border border-fourth py-3 rounded-md" type="tel" wire:model='phone'>
                    @error('phone')
                ...
                <div class='flex flex-col mb-4'>
                    ...
                    <input name='email' class="border border-fourth py-3 rounded-md" type="email" wire:model='email'>
                    @error('email')
                    <p class="text-red-500 text-xs italic mt-4">
                        {{ $message }}
                    </p>
